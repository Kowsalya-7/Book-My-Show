pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {
            steps {
                git branch: 'main', url: 'https://github.com/Kowsalya-7/Book-My-Show.git'
                sh 'ls -la'
            }
        }
        ...
        stage('Quality Gate') {
            steps {
                script {
                    waitForQualityGate abortPipeline: true, credentialsId: 'kowsalya_sonarqube'
                }
            }
        }
        ...
        stage('Deploy to Container') {
            steps {
                sh '''
                    ...
                    docker run -d --restart=always --name bms -p 3000:3000 kowsalyaganta/bms:latest
